...
class QScraper:
    ...
    def process_one(self, con, src_table, rid, url_name='url'):
        ...
        d_res = self.__fnproc(xres, rid)
        self.save_result(con, d_res)
        return 0
    
    def get_processor(self, con, src_table, url_name='url'):
        return lambda x: self.process_one(con, src_table, x, url_name=url_name)
    
    def run_process(self, con, q_name, src_name, log_name,
        ...
        while True:
            res = qp.queue_process(con, q_name, log_name, self.get_processor(con, src_name))
